...
htheta2= -math.degrees(math.acos((hx*hx+hy*hy-(l1*l1)-(l2*l2))/ (2*l1*l2)))
htheta1= math.degrees(math.asin((l2*math.sin(htheta2*math.pi/180))/math.sqrt(hx*hx+hy*hy)))+ math.degrees(math.atan(hy/hx))
...
##
oldtheta2=-math.degrees(math.acos((ox*ox+oy*oy-(l1*l1)-(l2*l2))/ (2*l1*l2)))
oldtheta1=math.degrees(math.asin(l2*math.sin(oldtheta2*math.pi/180)/math.sqrt(ox*ox+oy*oy)))+ math.degrees(math.atan(oy/ox))
...
theta2=-math.degrees(math.acos((x*x+y*y-(l1*l1)-(l2*l2))/ (2*l1*l2)))
theta1=-math.degrees(math.asin(l2*math.sin(theta2*math.pi/180)/math.sqrt(x*x+y*y)))+ math.degrees(math.atan(y/x))
...
_thread.start_new_thread( print_time, ("stepper-1", 0.1, s1,abs(step1),dir1,td1))
_thread.start_new_thread( print_time, ("stepper-2", 0.1, s2,abs(step2),dir2,td2))
